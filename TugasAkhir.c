...
BSTNode* insertBST(BSTNode* root, Arsip arsip) {
   if (root == NULL) {
   ...
   if (strcmp(arsip.nama, root->data.nama) < 0)
      root->left = insertBST(root->left, arsip);
   else
      root->right = insertBST(root->right, arsip);
   return root;
}
...
void inorderBST(BSTNode* root) {
   if (root != NULL) {
      ...
      printf("ID: %d, Nama: %s, Tanggal: %s, Kategori: %s\n",
         root->data.id, root->data.nama, root->data.tanggal, root->data.kategori);
...
void tambahArsip(Arsip arsip) {
   ...
   rootBST = insertBST(rootBST, arsip);
}
...
void editArsip() {
   ...
   while (temp != NULL) {
      if (temp->data.id == idEdit) {
         ...
         input[strcspn(input, "\n")] = '\0';
         if (strlen(input) > 0) strcpy(temp->data.nama, input);

         printf("Tanggal [%s]: ", temp->data.tanggal);
         fgets(input, 20, stdin);
         input[strcspn(input, "\n")] = '\0';
         if (strlen(input) > 0) strcpy(temp->data.tanggal, input);
...
int main() {
   ...
   do {
      ...
      printf("3. Cari Arsip\n");
      printf("4. Undo Tambah Arsip\n");
      printf("5. Proses Antrian \n");
      printf("6. Tampil Antrian \n");
      printf("7. Edit Identitas Arsip (ID)\n");
      printf("8. Hapus Arsip\n");
      printf("9. Tampilkan BST (Nama Terurut)\n");
      printf("0. Keluar\n");
      printf("Pilih: ");
      scanf("%d", &pilihan);
      getchar();
      ...
      switch (pilihan) {
         ...
         break;
         case 9:
         printf("\n--- Arsip dalam BST (berdasarkan nama) ---\n");
         inorderBST(rootBST);
         break;
      }
   } while (pilihan != 0);
